...
- `OSC`: A/B対称の波形、位置、レベル、ユニゾン、デチューン、ステレオ幅、音程。WAVEはBasic Shapes / Analog Sweep / Digital Edge / Hollow Formantの4領域、POSは各4フレームを連続移動する。各波形直下のWarpは`OFF / BEND / ASYM / SYNC`と符号付きAmountをA/B別に持ち、AmountをMatrixから変調できる。SYNCはstretch / compressと0.25〜4倍ratioを状態文へ示す。実値から描くENV 1 / ENV 2 / LFO 1 / LFO 2設定図と、SUB / NOISE / FM、FILTER。LFO 1は直接送りとMatrix、LFO 2はMatrix専用の独立sourceとして使う。
...
- `MATRIX`: コアと同じ12 source選択（None + 11信号）、16 destination選択（None + 15送り先）、6 slot。SOURCEを選んでOSC面の`+ MOD`を押す方法と、行を直接編集する方法は同じパラメータ55〜72へ接続される。Macro 1〜4と独立Mod EGをsourceにでき、A/B別Warpも送り先にできる。
...
PCキー`A W S E D F T G Y H U J K`または画面鍵盤の最初の操作でAudioContextを開始し、その操作自体も発音になる。PERFORMANCE帯の`MODE`は既定の`POLY`、一番後に押した音を優先する`MONO`、保持中はAMP EGを鳴らし直さない`LEGATO`を選べる。MONOは`GLIDE = 0`のときだけ重ねた新ノートで再アタックし、Glide中は現在の包絡を保って音程だけを移動する。`GLIDE`は0〜2秒で、保持中の音程遷移だけを滑らかにし、新しいフレーズはただちに目標音程から始まる。`VOICES`を含むinteger指定パラメータは数値入力とpatch復元の両方で整数へ丸める。`Z`で1 octave下、`X`で1 octave上へ移動し、範囲は-3〜+3 octave。移動前に押下中の全noteを解放し、画面鍵盤と現在音域表示を一緒に更新する。`STOP SOUND`はpointerdown時点で、`Esc`とキーボード／支援技術によるclickは同じ経路で、押下中／待機中の入力を消去し、voice resetを要求してから出力ゲートを現在時刻の0へ固定し、AudioContextを休止する緊急停止である。再開は次の有効な鍵盤入力だけで行う。ダイヤルは上下ドラッグ、Shift併用の微調整、ダブルクリックの初期値復帰、単位付き数値入力、rangeのキー操作に対応する。文字入力欄とselectへ入力中だけPC鍵盤演奏を抑止する。

`?quality=1`を付けた比較画面だけに`4-VOICE DENSITY`を表示する。`2.0 FOCUS / 3.0 BALANCED / 4.0 FULL / 5.0 WIDE+`の4ボタンでA/Bのdensityを連動操作し、4.0を標準値とする。表示値はエネルギー寄与であり、小数個の発音声や最大同時発音数ではない。4 unison時の内側2声を芯とし、2.0は内側ペアだけ、4.0は従来と完全一致、5.0は実声数やpan位置を増やさず外側ペアを約22.5%強める。音量は連続的に正規化し、通常画面には表示せず、UNISONが1〜3のときは音へ影響しない。
...
共有Insertはコア内部でwet/bypassを平滑化する。AudioContext停止中はDelay入力、Reverb mixなどWeb側AudioParamを予約補間せず即時値で初期化する。出力は0から25 msで開く安全ゲートを通し、初回の過大出力を防ぐ。緊急停止時はその予約を取り消し、ゲートを`setValueAtTime(0, currentTime)`で明示的に閉じてからAudioContextを休止する。画面鍵盤はpointer IDごとに発音を管理し、`pointerup` / `pointercancel` / `lostpointercapture`で解除する。ウインドウのblur、pagehide、非表示化でも同じ停止経路を使う。次の演奏入力で同じAudioContextを再開できる。
...
パッチは`schemaVersion: 1`で、core ID 0〜124（90〜112の共有Insert値は`fx`を正本として除外）、共有Insertの`fx`値と順序、Web空間系をまとめる。既存10音色に加え、ローカル保存は最大8件。自動復元、検索／カテゴリ、Undo / Redo、JSON EXPORT / IMPORTがある。旧76／79／83／90／113／117／119／121値パッチは不足値を既定値で補い、`fx`を正本として読み込む。破損JSONは適用前に拒否し、既存状態を保つ。ユーザー保存と自動復元はこのページの`localStorage`内だけで、外部送信しない。

Patch Toolsの`PRESET BRIDGE`は、Serum 2 `.SerumPreset`をローカルCLIでschema 1 JSONへ変換する手順を開く。変換は確認できた数値だけを反映し、完全互換を主張しない。他社wavetable / sampleは転送しない。Modulationは実画面で補正したENV 1〜3 / Macro 1〜4のlinearな単一source routeだけを既存6-slot Matrixへ最大6件移し、PianofyではENV 2からNoise Levelも開く。Aux、bipolar、curve、LFO、未知destinationと残りFXはmapping reportへ残す。VoiceFilter Driveと確認できたTape SatはDrive 0.24以下・Mix 0.20以下のDistortionへ安全側に近似し、Fat値は推測で補わない。判断ページの固定3候補は`SYNTHENGINEで開く`から対応patchを一手で読み込める。候補IDは固定allowlistで、明示選択をautosaveより優先し、成功後にqueryを除く。自動発音はせず、鍵盤またはPCキーまで出力をミュートする。`SERUM ORIGINAL`はlocalhostサーバーの固定3件allowlistからインストール済み原本を`.SerumPreset`名でdownloadでき、`JSON保存 / MAPPING`も別入口として残す。原本をworkspaceや公開buildへ複製せず、任意パスやURLも受けない。Serum 1 `.fxp`、Massive `.nmsv`、Avengerはsampleと内部形式の確認待ち。
...
Studio UIの保存・操作面とDelay / ReverbはWeb殻に置き、共有InsertはM4nからコア／AUにも公開する。M4aではコアの内蔵4 wavetableを各4フレームへ拡張し、C ABIとパラメータ数を維持したままengine versionを9へ更新した。M4fではmip境界をalias-safeな100 cent crossfadeへ変更し、同じ境界を通るWeb / nativeコアをengine version 10へ更新した。M4gではMorph／FM／Level系8操作の5 ms平滑化と、unisonのdetune／pan配置分離をコアへ追加しengine versionを11へ更新した。M4hではBalanced位相、Natural Widthカーブ、高域FM Guardを比較用の3パラメータとして追加し、engine versionを12へ更新した。M4iでは日常のOSC操作を先頭へ戻し、`QUALITY LAB`は通常画面から外して`?quality=1`の明示的な検証URLだけに残した。M4jではAudioContext再開直後のWeb FX初期値と入力解放を安全化し、M4kではReverb IRの過大ゲインと自動復元時のプリセット表示ずれを修正した。M4lではMatrix専用の独立LFO 2を追加し、既存IDを維持したまま83パラメータ・engine version 13へ更新した。M4mではMacro 3 / 4と独立Mod EGを追加して90パラメータ・engine version 14へ、M4nでは4 Insertと順序を共通コアへ移して113パラメータ・engine version 15へ更新した。M4rではslot 4をセッション用Customへ拡張し、厳密なローカルWAV読込と非破壊検証を追加してengine versionを16へ更新した。M4sではZ/X octave移動とCustom WTのREPLACE/CLEARをWeb UIへ追加し、M4tではCustom使用中だけA/B別のframe位置を既存帯へ追加した。M4uでは既存HQ Guardだけの高域FM深さを1.5倍まで回復し、C8・100% FMの分析でLegacyより折返し成分を22.47 dB抑えたまま、深さを0.251から0.377へ戻した（engine version 17）。M4ahでは末尾ID 113 / 114としてPOLY / MONO / LEGATOとGlideを追加し、既存の初期値をPOLY / 0秒のままengine versionを21へ更新した。M4ajでは本人試聴に基づきGlide中のMONO再アタックを外し、Glide 0のMONOだけに再アタックを残してengine versionを22へ更新した。M4akでは末尾ID 115 / 116として4声時だけのA/B別Unison Densityを追加し、既定値1で従来4声を維持したままengine versionを23へ更新した。M4alでは比較を4段切り替えへ固定し、実声数を増やさない5.0 WIDE+まで上限を拡張してengine versionを24へ更新した。M4amでは末尾ID 117 / 118にA/B別のBend Warpを追加し、既定OFFの旧PCMを保ったままalias-aware mipと5 ms平滑化を通してengine versionを25へ更新した。M4anでは通常OSC A／Bの波形直下へ`OFF / BEND` modeと符号付きAmountを出し、A/B独立操作とQuality Lab連動表示を追加した。M4aoでは既存6 slotへA/B別Warp destinationを末尾追加し、通常OSCのAmount横からも同じrouteを編集できるようにしてengine versionを26へ更新した。M4apでは末尾ID 119 / 120へA/B別Warp modeを追加し、既定BENDを維持したまま単調なASYM変形と5 msのmode補間を加えてengine versionを27へ更新した。M4aqでは同じmode IDへSYNCを追加し、master周期resetをPolyBLEP補正、mode切替を3-way one-hot補間へ変更してengine versionを28へ更新した。M4arではSerum 2用のローカルpreset bridgeとmapping reportを追加し、M4avでは実画面でsource番号を補正してNoise routeと安全上限つきDrive / Tape Sat近似を追加した。どちらもDSP・schema・engine versionは変更していない。M4awでは既存3-band EQを160 Hz low shelf／1.2 kHz Q 0.75 mid bell／6.8 kHz high shelfの直列biquadへ更新し、発音中の3 gainを5 ms平滑化、0 dBの旧PCMと121パラメータを維持してengine versionを29へ更新した。M4axでは末尾ID 121〜124へLOW / MID / HIGH FrequencyとMID Qを追加し、7操作を発音中5 ms平滑化、同じRBJ係数からWeb response curveを表示して125パラメータ・engine version 30へ更新した。M4s/M4t/M4an/M4ar/M4avはコアとengine versionを変更していない。比較時は従来どおりユニゾンとFMを独立操作でき、Studio全体の44px操作面積とコンパクト密度も維持する。設計メモと数値トークンは`design/SYNTH_UI_DESIGN.md`、`design/SYNTH_UI_TOKEN_SHEET.md`に置く。2026-09-07の比較音と検証台帳は`design/overnight-runs/2026-09-07-serum-until-08/`に置く。
...
発音・DSP・Web Audioに触れる変更では、先に現在のWASMをビルドしてから79件のWebテストを実行する。
`audio-safety.test.mjs`はAudioWorkletProcessorへ実WASMを読み込み、出力をAudioContextや物理デバイスへ接続せず、
48 kHz／128 framesでCustom WT読込、初回発音、POLY→MONO→LEGATOとGlide中の保持ノート遷移、Morph変更、4 unisonのDensity 2.0→約3.0→4.0、保持中のAMP／FILTER Sustain・FILTER ENV AMOUNT、FILTER MODE（LP12 / BP12 / HP12 / NOTCH / LP24 / HP24）とFILTER ON/BYPASS切替中の発音継続、note-off後の無音、voice resetによるpanic後の無音、CLEAR用安全波形の無発音再読込、
全サンプルの有限性と低出力プリセット時のpeak上限0.25を検査する。3音×4 unisonの保持中にCutoff／Resonance／Key Track／Filter ENV／Filter EG／per-note overrideとLFO 1→Cutoff・LFO 2→ResonanceのMatrixを密に切り替えるFILTERゲートは、同じ操作列に3-band EQのGain / Frequency / Q両端切替も含めて44.1／48／96 kHzの各rateで検査する。48 kHzではblock直前・境界・直後の127／128／129、255／256／257、511／512／513 frameへ同一frameのFILTER操作束とper-note設定→note-onを置き、出力の上限とrelease／panic後無音を別途確認する。加えて44.1／48／96 kHzでは、旧processorを明示resetした後の無音、新しいprocessorの初期無音、C8・HQ FM保持音の有限性と上限を検査し、processor間に保持音を持ち越さないことを固定する。これは物理出力未接続の再生成シミュレーションであり、実ブラウザのオーディオデバイス切替そのものは証明しない。WASMが無いローカル実行ではこれらの実WASM検査をskipするが、
...
| 項目 | 値 | 環境・備考 |
|---|---:|---|
| WASM raw | 85,792 B | `wc -c build/synth_engine.wasm`、2026-09-09 M4am実測 |
| WASM gzip | 24,472 B | gzip圧縮、2026-09-09 M4am実測 |
