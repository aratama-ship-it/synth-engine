...
| 38 | filterResonance | 0..1 | 0 | 共振。1 で Q=100（強く鳴くが持続的な自己発振はしない） |
...
k  = 2 - 1.99 * resonance       resonance 0 → k=2 (Q=0.5) ／ 1 → k=0.01 (Q=100)
a1 = 1 / (1 + g * (g + k))
a2 = g * a1
a3 = g * a2
...
26. **高共振の安定**: `resonance=1`・入力なし・10秒。NaN/Inf 0、`|sample| <= 8`。
    ★この項目は「暴れないこと」しか見ていない（入力ゼロなら線形フィルタの出力はゼロのままなので当然通る）。
    共振が実際に鳴ることはテスト25（+10 dB 以上のピーク）で担保する。
    ★実測（2026-09-05・Claude）: カットオフ 500 Hz・resonance 1 でノイズを 20 ms だけ入れると、**500 Hz でリンギングし
    136 dB/秒 で減衰**した。これは Q=100 の理論値と一致する。**持続的な自己発振はしない**（k>0 のため）。
    さらにボイスのアンプEGが閉じるとリンギングも消える（フィルタはアンプEGの前段にあるため）。これは減算方式の通常の挙動。
